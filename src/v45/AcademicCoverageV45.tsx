...
import { computingErasV43, evidenceGatesV43, facultiesV43, grandChallengesV43, sourcePillarsV43, standardsV43 } from "../v43/curriculum-v43"
import KnowledgeGraphV43 from "../v43/KnowledgeGraphV43"
import ReviewEngineV43 from "../v43/ReviewEngineV43"
...
export default function AcademicCoverageV45({ completedStageCodes, currentStageIndex }: { completedStageCodes: string[]; currentStageIndex: number }) {
  ...
  return (
    <section className="v43-omni-layer" aria-labelledby="v43-omni-title">
      <div className="v43-omni-intro">
        <div><span>AUDITORÍA ACADÉMICA · COBERTURA DEL PLAN</span><h2 id="v43-omni-title">Un plan amplio necesita orden.<br /><em>Y pruebas de dominio.</em></h2></div>
        <p>El tronco S0–S19 se contrasta con estándares, planes universitarios, cuerpos de conocimiento, práctica profesional y líneas de investigación. El índice profesional sirve para localizar trayectorias sin convertir la formación en una colección de títulos de puesto.</p>
      </div>

      <div className="v43-omni-metrics">
        <article><small>MACRODOMINIOS</small><strong>{facultiesV43.length}</strong><span>Áreas de dominio</span></article>
        <article><small>PERFILES PROFESIONALES</small><strong>{careerCatalogV43.length.toLocaleString("es-PE")}</strong><span>Nombres y roles para orientar búsquedas</span></article>
        <article><small>MARCOS DE REFERENCIA</small><strong>{standardsV43.length}</strong><span>Estándares, BoK y planes base</span></article>
      ...
      <nav className="v43-omni-tabs" aria-label="Capas avanzadas de Ruta Maestra">
        <button className={tab === "atlas" ? "is-active" : ""} onClick={() => setTab("atlas")}><GraduationCap /> 18 áreas de dominio</button>
        <button className={tab === "grafo" ? "is-active" : ""} onClick={() => setTab("grafo")}><Sparkles /> Dependencias</button>
        <button className={tab === "tiempo" ? "is-active" : ""} onClick={() => setTab("tiempo")}><BookOpen /> Historia → frontera</button>
        <button className={tab === "evidencia" ? "is-active" : ""} onClick={() => setTab("evidencia")}><Target /> Pruebas + proyectos</button>
        <button className={tab === "repaso" ? "is-active" : ""} onClick={() => setTab("repaso")}><BookOpen /> Repaso FSRS</button>
        <button className={tab === "fuentes" ? "is-active" : ""} onClick={() => setTab("fuentes")}><ShieldCheck /> Jerarquía de fuentes</button>
      </nav>

      {tab === "atlas" && <div className="v43-tab-panel">
        <div className="v43-career-search">
          <div className="v43-career-search-copy"><span>BUSCADOR DE PERFILES</span><h3>Busca una profesión o especialidad del índice.</h3><p>El buscador localiza el nombre y lo vincula con su área académica principal. Es una ayuda de navegación: muchos perfiles requieren conocimientos de varias áreas.</p></div>
          <div className="v43-career-search-box"><div><Search /><input value={query} onChange={(event) => { setQuery(event.target.value); setSelectedCareer("") }} placeholder="Ej.: GPU Architect, Data Engineer, CISO, Quantum…" /></div>{results.length > 0 && <div className="v43-career-results">{results.map((role) => <button key={role} onClick={() => { setSelectedCareer(role); setQuery(role) }}><span>{role}</span><ArrowRight /></button>)}</div>}{mapped && <article className={`v43-career-map tone-${mapped.tone}`}><small>{mapped.code} · ÁREA PRINCIPAL</small><strong>{mapped.title}</strong><p>{mapped.description}</p><span>{mapped.stageRange}</span></article>}</div>
        </div>
      ...
      {tab === "evidencia" && <div className="v43-tab-panel">
        <div className="v43-panel-head"><div><span>PRUEBAS DE DOMINIO</span><h3>Avanzar exige producir evidencia.</h3></div><p>Cada tramo debe terminar en resultados observables: problemas resueltos, laboratorios reproducibles, sistemas construidos, auditorías, defensa técnica e investigación.</p></div>
        ...
        <div className="v43-challenge-head"><span>PROYECTOS INTEGRADORES</span><h3>Diez proyectos para combinar conocimientos de varias áreas.</h3></div>
      ...
      {tab === "fuentes" && <div className="v43-tab-panel">
        <div className="v43-panel-head"><div><span>JERARQUÍA DE FUENTES</span><h3>La ruta distingue autoridad de utilidad.</h3></div><p>A = estándar o plan oficial; B = universidad y material académico; C = currículo o laboratorio abierto; D = repositorio comunitario útil. Los niveles de menor autoridad complementan la práctica, pero no sustituyen las referencias oficiales.</p></div>
