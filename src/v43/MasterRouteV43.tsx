...
import OmniversityLayerV43 from "./OmniversityLayerV43"
import "./route-v43.css"
...
export default function MasterRouteV43() {
  ...
  const resetProgress = () => {
    if (!window.confirm("¿Reiniciar solo el progreso visual de Ruta Maestra V43? No modifica tus Recursos, Certificaciones ni otros datos del Campus.")) return
    ...
    <section ref={rootRef} className="v43-master-route" aria-labelledby="v43-route-title">
      <header className="v43-route-hero">
        <div className="v43-route-hero-copy">
          <span>RUTA MAESTRA 3.0 · OMNIVERSITY · MAPA ACADÉMICO VIVO</span>
          <h1 id="v43-route-title">Veinte etapas troncales.<br /><em>Un sistema para dominar la computación.</em></h1>
          <p>Del punto cero a la frontera: S0 → S19, especializaciones, 18 macrodominios, estándares internacionales, evidencia rigurosa, investigación, repaso inteligente y un mapa navegable sin abandonar el lenguaje visual de Campus Maestro.</p>
          <div className="v43-route-hero-actions"><button onClick={() => focusNode(`stage-${stages[currentIndex].code}`)}><Target /> Continuar en {stages[currentIndex].code}</button><button onClick={() => document.querySelector(".v43-route-map-shell")?.scrollIntoView({ behavior: "smooth" })}><Map /> Explorar mapa</button></div>
        </div>
      ...
      <OmniversityLayerV43 completedStageCodes={progress.completedStages} currentStageIndex={currentIndex} />
      ...
      <section className="v43-track-showcase">
        ...
        <div className="v43-track-rail"><div className="v43-track-strip">{[...masteryTracks, ...masteryTracks].map((track, index) => { const realIndex = index % masteryTracks.length; return <button key={`${track.code}-${index}`} className={`tone-${palette[realIndex % palette.length]}`} onClick={() => { setSelected({ kind: "track", index: realIndex }); if (!isMobile) focusNode(`track-${track.code}`) }}><small>{track.code} · {track.family}</small><strong>{track.title}</strong><span>{track.duration}</span><i>Explorar <ArrowRight /></i></button> })}</div></div>
      </section>
